...
for (metric in metrics) {
  ...
  model <- load_model_hdf5(file.path(model_dir, paste("model_", metric, ".hdf5", sep = "")))
  ...
  ggsave(file.path(model_dir, paste("roc_", metric, ".png", sep = "")))
  ...
  ggplot(df, aes(x=Id, y=Error, col=Anomaly)) + 
    ...
    geom_rug()
  ggsave(file.path(model_dir, paste("data_", metric, ".png", sep = "")))
  ggplot(df, aes(x=Error, fill=Anomaly)) +
    geom_density(alpha = 0.5)
  ggsave(file.path(model_dir, paste("density_", metric, ".png", sep = "")))
